...
<script id="proListTpl" type="text/html">
    ...
    <tr>
        ...
        <td>
            <a href="javascript:;" data-id="{{ item.proid }}" class="layui-btn layui-btn-normal layui-btn-mini proview">详细</a>
            {{#  if(item.changed == 0){ }}
                {{#  if(item.prostep == 11){ }}
                <a href="javascript:;" data-id="{{ item.proid }}" class="layui-btn layui-btn-mini supervisorLog"><i class="fa fa-upload"></i>&nbsp;&nbsp;监理日志上传</a>
                {{#  } }}
            {{#  } }}
        </td>
    </tr>
